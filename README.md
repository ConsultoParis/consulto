# 1Expert — Guide de mise en ligne

Ce projet est le squelette technique réel de 1Expert (Next.js + Supabase +
Stripe + Resend). Voici, dans l'ordre, tout ce qu'il faut faire pour passer
de ce code à un site en ligne sur **1expert.fr**.
...
Chez OVH, Gandi ou Namecheap : **1expert.fr**. Ne le connecte pas encore à
...
3. Une fois le site en ligne (étape 5), reviens configurer le **webhook** :
   Developers > Webhooks > Add endpoint → URL `https://1expert.fr/api/webhooks/stripe`,
...
> de 1Expert), en séquestre. Il ne transfère pas encore automatiquement la
...
> 1Expert, un peu comme sur Uber ou Airbnb) et calculer ta commission.
...
2. Ajoute et vérifie ton domaine `1expert.fr` (Resend te donnera des
...
4. Déploie
5. Dans Vercel, va dans **Domains** → ajoute `1expert.fr` → Vercel te donne
