...
function FieldCell({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="border-b p-3.5 sm:border-b-0 sm:border-r sm:last:border-r-0" style={{ borderColor: "var(--border)" }}>
      <label className="font-mono text-[10px] font-semibold uppercase tracking-[0.06em] text-mutedmore">{label}</label>
...
const fieldInputClass =
  "w-full rounded-[4px] border px-3 py-2 text-sm outline-none transition-colors focus:border-[#3E8EF7]";
const fieldInputStyle = { backgroundColor: "var(--input-bg)", borderColor: "var(--border)" };
...
export default function AddSlotForm({ expertId, profession }: { expertId: string; profession?: Profession }) {
  ...
  return (
    <form onSubmit={handleAdd} className="space-y-4">
      ...
      <div className="overflow-hidden rounded-[8px] border" style={{ borderColor: "var(--border)" }}>
        <div className="grid grid-cols-1 sm:grid-cols-3" style={{ backgroundColor: "var(--card)" }}>
          {recurrenceType === "unique" ? (
            ...
            <FieldCell label="Jour de la semaine">
              <select
                className={fieldInputClass}
                style={fieldInputStyle}
                value={weekday}
        ...
        <div className="border-t p-3.5" style={{ borderColor: "var(--border)", backgroundColor: "var(--card)" }}>
          <button type="submit" disabled={loading} className="btn-primary w-full rounded-[6px] py-2.5 text-sm font-medium sm:w-auto sm:px-8">
      ...
      {duration === 5 ? (
        ...
        <div>
          <label className="font-mono text-[11px] font-semibold uppercase text-mutedmore">Modes de consultation proposés</label>
