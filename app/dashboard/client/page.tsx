...
export default async function ClientDashboardPage() {
  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-16">
      ...
      <div className="mt-6">
        <LoyaltyProgressBar
          progressInCurrentTier={loyalty.progressInCurrentTier}
          discountsAvailable={loyalty.discountsAvailable}
        />
      </div>
            ...
            {bookings?.map((b: any) => {
              ...
              return (
                <div key={b.id} className="card-soft p-5" style={{ backgroundColor: "var(--card)" }}>
                  ...
                  {b.status === "completed" && (
                    <a href={`/api/bookings/${b.id}/receipt`} className="btn-secondary ml-2 mt-3 inline-flex items-center gap-1.5 rounded-[3px] px-4 py-2 text-xs font-medium"><Download className="h-3.5 w-3.5" /> Mon reçu (PDF)</a>
                  )}
